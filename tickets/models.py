...
# Если житель молчал в открытой заявке дольше этого срока и написал снова —
# заявка считается устаревшей: старая закрывается как выполненная, заводится
# новая (с собственным заголовком от классификатора). Иначе заголовок первой
# фразы («Привет») навсегда остаётся названием заявки, даже когда через
# неделю человек пишет по совсем другому вопросу. Общий порог для Telegram
# и WhatsApp — раздельного смысла не увидели.
